...
import com.spring.myapp.reply.controller.ReplyController;
...
public class UserController {
	...
	public String login(@RequestBody LoginVO login, HttpSession session, HttpServletResponse res) throws Exception{
		...
		// 1. 세션객체를 얻는 방법 HttpServletRequest 객체 사용
		//  -	request.getSession();
		...
		/*
		 * + 클라이언트가 전송한 id 값과 pw 값을 가지고 DB에서 회원의 정보를 조회해서 불러온다음 값비교를 통해 
		 * 1. 아이디가 없을 경우  클라이언트 측 문자열 idFail 전송 
		 * 2. 비밀번호가 틀렸을 경우 문자열 pwFail 전송 
		 * 3. 로그인 성공시 loginSuccess 전송
		 */
	      ...
	      if(user != null) {
	         ...
	         if(encoder.matches(login.getUserPw(), user.getUserPw())) {
	            ...
	            //session.setMaxInactiveInterval(60*60);//세션의만료시간 1시간으로 증가 
