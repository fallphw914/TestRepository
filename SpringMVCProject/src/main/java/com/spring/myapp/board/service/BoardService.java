...
public class BoardService implements IBoardService {
	...
	public List<BoardVO> listSearch(SearchCriteria cri) throws Exception {
		...
		// 1일 이내의 게시물에만 new를 붙이는 처리
		for (BoardVO boardVO : list) {
			// 게시물 목록을 요청한 시간 - 게시물 등록시간 < 1일 미만 => new 마크 생성
			
			long now =System.currentTimeMillis();//현재 시간(게시물 목록을 요청한 시간)
			//Date date = boardVO.getRegDate();  게시물 등록 시간
			long regDate = boardVO.getRegDate().getTime(); // regDate를 밀리초로 바꿔 계산 하기위함
			
			long oneDayMillis = 60 * 60 * 24 * 1000; //1일의 밀리초(=1000분의 1초)
